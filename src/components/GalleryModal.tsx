...
export default function GalleryModal({ selectedReview, onClose }: GalleryModalProps) {
  ...
  const previewAnimationVariants: any = isMobile
    ? {
        initial: { opacity: 0, scale: 0.95, y: 15 },
        animate: { opacity: 1, scale: 1, y: 0, transition: { type: 'tween', duration: 0.35, ease: 'easeOut' } },
        exit: { opacity: 0, scale: 0.95, y: 15, transition: { type: 'tween', duration: 0.3, ease: 'easeIn' } },
      }
    : {
  ...
  const formAnimationVariants: any = isMobile
    ? {
  ...
  return (
    <>
      {/* --- PREVIEW MODAL (ОКНО С ФОТО) --- */}
      <AnimatePresence>
        {selectedReview && (
          <div 
            style={{ position: 'fixed', top: 0, bottom: 0, left: 0, right: 0, zIndex: 80 }} 
            className="sm:flex sm:items-center sm:justify-center sm:p-4 font-paragraph text-foreground"
          >
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              ...
              className="absolute inset-0 bg-background sm:bg-black/50 sm:backdrop-blur-[2px]"
            />
                ...
                {isSubmittedSuccess ? (
                  ...
                  <>
                    ...
                    <div className="p-3 bg-foreground/5 rounded-xl flex gap-3 items-center border border-foreground/10 mb-4">
                      ...
                      <div>
                        <p className="font-heading text-xs sm:text-sm font-semibold text-foreground line-clamp-1">
                          {getProductName(selectedReview)}
                        </p>
                        <p className="font-heading text-xs font-bold text-soft-gold mt-0.5">
                          Custom Order
                        </p>
                      </div>
                    </div>

                    <form id="custom-order-form" onSubmit={handleSubmit} className="space-y-3.5 pb-2">
                      <div>
                        <Label
                          htmlFor="fullName"
